...
# career-ops batch runner — standalone orchestrator for hermes -p workers
# Reads batch-input.tsv, delegates each offer to a hermes -p worker,
...
usage() {
  cat <<'USAGE'
career-ops batch runner — process job offers in batch via hermes -p workers
Uses your default Hermes agent.
